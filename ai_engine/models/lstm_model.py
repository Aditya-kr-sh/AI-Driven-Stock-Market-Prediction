...
Implements the LSTM network module and the LSTMPredictor BasePredictor wrapper.
"""
...
class LSTMNet(nn.Module):
    """Core PyTorch LSTM network module."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Fetch the output representation of the last sequence step
        last_step = lstm_out[:, -1, :] # last_step shape: (batch_size, hidden_size)
        return self.fc(last_step)


class LSTMPredictor(BasePredictor):
    """
    LSTM Predictor wrapper matching the BasePredictor interface.
    ...
    def fit(self, train_data: Tuple[np.ndarray, np.ndarray], val_data: Tuple[np.ndarray, np.ndarray], **kwargs) -> dict:
        ...
        # Set shuffle=False to respect temporal chronological order strictly
    ...
    def predict(self, data: np.ndarray) -> np.ndarray:
        ...
        if data.ndim == 2:
            # tab format to sequence shape: (1, seq, features)
    ...
    def load(self, filepath: str) -> None:
        ...
        if self.input_dim is None:
            raise ValueError("Could not reconstruct LSTM network. input_dim missing in metadata.")
